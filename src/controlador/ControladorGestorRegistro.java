...
public class ControladorGestorRegistro implements Initializable {
	...
	@FXML
	void pulsarBtnAceptar_reg(ActionEvent event) throws ControladorExcepciones {
		...
		// guardar dia y hora de la cita en un String
		String calend = dia + "-" + mes + "-" +anho;
		...
		if (comprobarDigitosDNI() == true ) {
			if (pswrd.compareTo(pswrdSecond) == 0) {
				...
				if (roltype == 0) {
					ControladorAvisos.setMensajeError("Seleccione el tipo de usuario.");
					abrirVentanaAvisos();
				}
				switch (roltype) {
					...
					case 1:
						try {
							...
							Paciente newPaciente = new Paciente(textoDNI.getText(), textoNombre.getText(), textoApellidos.getText(), textoTelefono.getText(), passwordEncriptada, calend, "62544518C", false);
							...
							ControladorAvisos.setMensajeError(
									"Error registrando paciente, por favor revise los datos introducidos.");
							abrirVentanaAvisos();
						}
						break;
					...
					case 2:
						try {
							...
							ControladorAvisos.setMensajeError(
									"Error registrando cuidador, por favor revise los datos introducidos.");
							abrirVentanaAvisos();
						}
						break;
					...
					case 3:
						try {
							...
							ControladorAvisos.setMensajeError(
									"Error registrando medico, por favor revise los datos introducidos.");
							abrirVentanaAvisos();
						}
						break;
				...
				ControladorAvisos.setMensajeError("Las contraseñas no coinciden.");
				abrirVentanaAvisos();
			}
		}else {
			ControladorAvisos.setMensajeError("El DNI introducido es erróneo.");
			abrirVentanaAvisos();
		}
	}
